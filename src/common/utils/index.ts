...
export default {
     ...
     */
    lpad(str: string, padLen: number, padStr: string | unknown[]) {
    ...
    formatBytes: (byte: number, digit: number = 2) => {
        ...
        return (
            parseFloat((byte / Math.pow(c, f)).toFixed(digit)) + ' ' + unit[f]
        )
    },
     ...
     * @returns 날짜 string
     */
    dateFormat(date: never, format: string) {
     ...
     */
    getTableSelection(tableRef: Ref, val: unknown[]): unknown[] {
        const hasMultiple = Object.keys(tableRef.value?.$attrs).includes(
            'multiple',
        )
        let resData = []
        if (hasMultiple) {
    ...
    excelDownload(
        ...
        fileDownload(
            data,
            `${fileName}_엑셀파일_${dayjs(new Date()).format('YYYY_MM_DD_HH_mm_ss')}.xlsx`,
        )
    },
    /**
